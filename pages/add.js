...
const add = () => {
    ...
    const unregisterAuthObserver = firebase
      ...
      .onAuthStateChanged((user) => {
        if (user) {
          ...
          console.log("add.js useEffect: allowedEmails", allowedEmails);
          ...
          if (allowedEmails.includes(user.email)) {
            ...
            setIsSignedIn(false);
            ShowErrorMessage(
              "You cannot access this page. You must add your email. Sorry."
            );
  ...
  const onSubmit = async (data) => {
    console.log("add.js onSubmit: data", data);
    ...
    if (success) {
      ...
      ShowErrorMessage(
        "Error occuring when trying to attempt add the code snippet to database. 😭"
      );
    }
  };
  ...
  return (
    ...
    >
      ...
      <div className={styles.buttonContainer}>
        <ActionButton disabledState={isSubmitting} buttonText="Add" />
      </div>
      ...
      <div className="w-100 mt-3">
        <p className="mb-0">
          ...
          You are signed in as {firebase.auth().currentUser.displayName}
        </p>
      </div>
    </Form>
  );
